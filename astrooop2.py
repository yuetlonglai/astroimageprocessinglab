...
if __name__ == '__main__':
    ...
    ax2.errorbar(m,logN,yerr=logNerr,fmt='.',marker = 's',color='black',markerfacecolor = 'none',label='All objects',capsize=2,elinewidth=1,markersize=4)
    ax2.plot(m,plogN(m),'-',color='black',label='All Object Fit')
    ax2.errorbar(mg,logN2,yerr=logNerr2,fmt='.',marker = '^', markerfacecolor = 'none',color='red',label='Galaxies',capsize=2,elinewidth=1,markersize=4,alpha=0.75)
    ax2.plot(mg,plogN2(mg),'-',color='red',alpha=1, label='Galaxies Fit')

    ax2.set_ylim(0,4)
    ax.set_ylim(ax.set_ylim(10**np.array(ax2.get_ylim())  ))
    ax2.set_yticklabels([])
    ax2.set_ylabel("")
