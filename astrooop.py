...
class Process:
    ...
    def mask_edge(self, cut):
        ...
        self.img = np.pad(cutimg, ((cut,cut),(cut,cut)), mode='constant', constant_values = (self.average_background))
        
        # self.show_img(np.where(cutimg not in self.img, self.average_background, self.img))
...
img.clean_bleeding(bleeding,clean_background=False)

lp = img.circle_lowpass(1200, 0.01)
lp2 = img.circle_lowpass(6,1)
...
# img.show_img()
y,x = img.identify_objects()
fluxlist = img.aperture_photometry()
# print(fluxlist)
plt.figure(figsize=(10,8))
plt.imshow(img.img,cmap='inferno',norm=colors.LogNorm())
plt.plot(x,y,'x',color='yellow',alpha=0.2)
plt.show()

counting = img.number_count()
